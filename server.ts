...
app.post("/api/gemini/evaluate-habits", async (req, res) => {
  ...
  if (ai) {
    try {
      const prompt = `أنت طبيب نفسي ومحلل سلوكي إكلينيكي خبير باللغة العربية.
قم بتحليل الالتزام بالعادات الشخصية للمستخدم خلال الفترة المحددة وتقديم تقرير تقييمي عميق ومحفز نفسياً.

العادات المسجلة وتفاصيل التزامها اليومي:
${formattedHabits}

الفترة المحددة للتقييم: ${period} (من ${startDate} إلى ${endDate})
نسبة الالتزام الإجمالية المحسوبة: ${overallCompliance}%

يرجى تزويدي بتقرير نفسي سلوكي منسق بـ Markdown يحتوي على الأقسام التالية:
1. 📈 **قراءة تحليلية سلوكية**: تعليق عام مفصل على التزام المستخدم بالعادات ونمط الانضباط الذاتي لديه.
2. 🧠 **الرابط النفسي والعصبي**: كيف يؤثر الالتزام بهذه العادات المحددة (مثل الرياضة أو التأمل أو شرب الماء) على الصحة النفسية، مستويات التوتر، والقدرة الذهنية للمستخدم علمياً.
3. ⚠️ **رصد العقبات والتحديات**: تحديد العادات الأقل التزاماً واقترح الأسباب النفسية وراء ذلك (مثل التسويف، وضع أهداف عالية، غياب المحفزات) وكيفية معالجتها.
4. 🚀 **خطة عمل مخصصة للمرحلة القادمة**: 3 توصيات عملية، ذكية ومحددة (SMART) لزيادة الالتزام وتحويل العادات إلى روتين تلقائي مدعوم بنظام المكافأة الدماغية.

تحدث بأسلوب دافئ، مشجع، علمي، ومتعاطف كمعالج نفسي صديق. أرجع تقريراً منسقاً بشكل ممتاز ومنظم جداً وبدون أي كلام جانبي خارجي.`;

      const response = await ai.models.generateContent({
        model: "gemini-3.5-flash",
        contents: prompt,
        config: {
          systemInstruction: "أنت مستشار سلوكي وطبيب نفسي خبير باللغة العربية، تحلل التزام العادات وتأثيرها على الصحة النفسية للمستخدم.",
        }
      });
...
### 3. ⚠️ رصد العقبات والحلول المقترحة
  ...
  if (ai) {
    try {
      ...
      if (reportType === "therapist") {
        userPrompt = `قم بإنشاء تقرير احترافي ومنظم موجه خصيصاً للمعالج النفسي أو الطبيب المتابع للحالة بناءً على كافة البيانات والمعلومات المتوفرة بالتطبيق للفترة من ${startDate || "البداية"} إلى ${endDate || "النهاية"}:

بيانات اليوميات، الخواطر، تمارين CBT، الأدوية والنشاط البدني:
${formattedDiaries || "لا توجد مذكرات أو يوميات مسجلة للفترة المحددة."}

العادات السلوكية وتاريخ الالتزام بها:
${formattedHabits || "لا توجد عادات سلوكية مضافة حالياً."}

قائمة الكتب والمصادر الملخصة والمقروءة:
${formattedBooks || "المكتبة فارغة حالياً."}

مفكرة الامتنان وبطاقات الشكر:
${formattedGratitude || "لا توجد بطاقات مسجلة حالياً."}

يرجى إخراج تقرير مفصل ومنظم للغاية باللغة العربية يحتوي على الأقسام التالية:
1. 📝 ملخص سريري شامل للفترة الحالية وسياقها العام.
