...
export default function DailyProsConsModal({
  ...
  const generateAiAnalysis = async () => {
    ...
    if (!fullCompiledContext || fullCompiledContext.trim().length === 0) {
      setAiPositives([isAr ? "لم يتم تدوين مذكرات تفصيلية أو أنشطة لليوم لتوليد الإيجابيات تلقائياً." : "No detailed entries or activities logged for today to auto-generate positives."]);
      setAiNegatives([isAr ? "لا توجد سلبيات مرصودة نظرًا لعدم وجود بيانات مسجلة لليوم." : "No negatives detected as there are no logged data for today."]);
...
- ...`,
  ...
  const parseAiText = (text: string) => {
    ...
    setAiPositives(posList.length > 0 ? posList : [isAr ? "استخدام التدوين والتعبير عن الذات بوعي وشجاعة." : "Using journaling and self-expression with mindfulness and courage."]);
    setAiNegatives(negList.length > 0 ? negList : [isAr ? "وجود ضغوط عابرة تم توثيقها بنجاح للتعامل معها." : "Encountering passing stressors that were successfully documented to handle."]);
  };
  ...
  const handleSave = () => {
    ...
    showToast(isAr ? 'تم حفظ سجل الإيجابيات والسلبيات بنجاح 💾' : 'Pros & Cons record saved successfully 💾');
    if (onSaveSuccess) onSaveSuccess();
  };

  // Copy Summary
  const handleCopy = () => {
    const summary = `⚖️ ${isAr ? 'تقرير الإيجابيات والسلبيات اليومية' : 'Daily Pros & Cons Report'} - ${displayDate}
----------------------------------------
🧠 ${isAr ? 'الإيجابيات المولدة بالذكاء الاصطناعي:' : 'AI-Generated Positives:'}
${aiPositives.map(p => `• ${p}`).join('\n')}

🧠 ${isAr ? 'السلبيات المولدة بالذكاء الاصطناعي:' : 'AI-Generated Negatives:'}
${aiNegatives.map(n => `• ${n}`).join('\n')}

✍️ ${isAr ? 'الإيجابيات المدخلة يدوياً:' : 'User Manual Positives:'}
${userPositives.length > 0 ? userPositives.map(p => `• ${p}`).join('\n') : (isAr ? '• لم يتم إدخال نقاط خاصة' : '• No user entries')}

✍️ ${isAr ? 'السلبيات المدخلة يدوياً:' : 'User Manual Negatives:'}
${userNegatives.length > 0 ? userNegatives.map(n => `• ${n}`).join('\n') : (isAr ? '• لم يتم إدخال نقاط خاصة' : '• No user entries')}

---
${isAr ? 'تم التوليد بواسطة تطبيق حياة AI 🌿' : 'Generated by Hayat AI 🌿'}`;
  ...
  const handleDownloadWord = () => {
    const content = `
      ...
      <head><meta charset='utf-8'><title>${isAr ? 'تقرير الإيجابيات والسلبيات' : 'Pros & Cons Report'} - ${displayDate}</title></head>
      <body style="font-family: Arial, sans-serif; direction: ${isAr ? 'rtl' : 'ltr'}; text-align: ${isAr ? 'right' : 'left'}; padding: 20px;">
        <h1 style="color: #2B3E50;">⚖️ ${isAr ? 'تقرير الإيجابيات والسلبيات اليومية' : 'Daily Pros & Cons Report'}</h1>
        <h3>${isAr ? 'التاريخ:' : 'Date:'} ${displayDate}</h3>
        <hr/>
        <h2 style="color: #2D5A27;">🧠 ${isAr ? 'الإيجابيات المولدة بالذكاء الاصطناعي:' : 'AI-Generated Positives:'}</h2>
        <ul>${aiPositives.map(p => `<li>${p}</li>`).join('')}</ul>
        
        <h2 style="color: #902923;">🧠 ${isAr ? 'السلبيات المولدة بالذكاء الاصطناعي:' : 'AI-Generated Negatives:'}</h2>
        <ul>${aiNegatives.map(n => `<li>${n}</li>`).join('')}</ul>
        <hr/>
        <h2 style="color: #2D5A27;">✍️ ${isAr ? 'الإيجابيات المدخلة يدوياً:' : 'User Manual Positives:'}</h2>
        <ul>${userPositives.map(p => `<li>${p}</li>`).join('')}</ul>

        <h2 style="color: #902923;">✍️ ${isAr ? 'السلبيات المدخلة يدوياً:' : 'User Manual Negatives:'}</h2>
    ...
    link.href = url;
    link.download = `${isAr ? 'تقرير_الإيجابيات_والسلبيات' : 'Pros_Cons_Report'}_${dayKey}.doc`;
    link.click();
    URL.revokeObjectURL(url);
    showToast(isAr ? 'تم تحميل التقرير كـ مستند Word 📝' : 'Report downloaded as Word doc 📝');
  };
