...
import { DiaryEntry, Habit, GratitudeCard, Book, AppLanguage } from '../types';
...
export default function ProsConsHistoryLog({
  ...
  const handleDeleteRecord = (dayKey: string) => {
    if (window.confirm(isAr ? "هل أنت متأكد من رغبتك في حذف سجل هذا اليوم من الإيجابيات والسلبيات؟" : "Are you sure you want to delete this day\x27s Pros & Cons record?")) {
      const updated = records.filter(r => r.dayKey !== dayKey);
      saveAllProsConsRecords(updated);
      setRecords(updated);
      showToast(isAr ? "تم حذف السجل اليومي بنجاح." : "Daily record deleted successfully.");
    }
  };
  ...
  const handleExportAllWord = () => {
    if (records.length === 0) {
      alert(isAr ? "لا توجد سجلات مسجلة بعد للتصدير." : "No records logged yet for export.");
      return;
    }

    const content = `
      <html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'>
      <head><meta charset='utf-8'><title>${isAr ? "سجل الإيجابيات والسلبيات الشامل" : "Comprehensive Pros & Cons History"}</title></head>
      <body style="font-family: Arial, sans-serif; direction: rtl; text-align: right; padding: 25px;">
        <h1 style="color: #2B3E50; text-align: center;">⚖️ ${isAr ? "سجل الإيجابيات والسلبيات الشامل لجميع الأيام" : "Comprehensive Pros & Cons Log for All Days"}</h1>
    ...
    link.href = url;
    link.download = isAr ? "سجل_الإيجابيات_والسلبيات_الشامل.doc" : "Pros_and_Cons_Comprehensive_Log.doc";
    link.click();
    URL.revokeObjectURL(url);
    showToast(isAr ? "تم تحميل السجل الكامل كـ Word 📝" : "Exported full record as Word document 📝");
  };
    ...
    <div className="space-y-6 font-sans text-right" dir="rtl" id="pros-cons-history-panel">
      ...
      <div className="bg-gradient-to-l from-[#2B3E50] via-[#3B5066] to-[#5A5A40] text-white p-6 rounded-3xl shadow-md border border-[#E2DCC8]/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-xl font-black text-white">
                {isAr ? "⚖️ سجل الإيجابيات والسلبيات اليومية" : "⚖️ Daily Pros & Cons Log"}
              </h3>
              <span className="bg-[#8B9D83] text-white text-[10px] font-black px-2.5 py-0.5 rounded-full border border-white/20">
                {isAr ? "سجل تاريخي كامل 🧠" : "Full History Archive 🧠"}
              </span>
            </div>
            <p className="text-xs text-[#E2DCC8] font-medium mt-1 leading-relaxed max-w-xl">
              {isAr ? "أرشيف وتوثيق زمني لجميع نقاط القوة والفرص والإيجابيات، إلى جانب التحديات والسلبيات المسجلة آلياً بالذكاء الاصطناعي أو يدوياً عبر الأيام." : "Chronological archive documenting strengths, opportunities, positives, and challenges logged automatically by AI or manually over time."}
            </p>
          </div>
        </div>
        ...
        >
          ...
          <span>{isAr ? "تصدير السجل الكامل (Word / PDF) 📄" : "Export Full Log (Word / PDF) 📄"}</span>
        </button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white border border-[#E2DCC8] rounded-2xl p-4 shadow-3xs flex items-center justify-between">
          <div>
            <p className="text-[11px] font-extrabold text-gray-500">{isAr ? "إجمالي الأيام المسجلة" : "Total Days Tracked"}</p>
            <p className="text-xl font-black text-[#2B3E50] mt-0.5">{totalDaysTracked} {isAr ? "أيام" : "days"}</p>
          </div>
        ...
        <div className="bg-[#F2F7F2] border border-[#C2DCBE] rounded-2xl p-4 shadow-3xs flex items-center justify-between">
          <div>
            <p className="text-[11px] font-extrabold text-emerald-800">{isAr ? "مجموع الإيجابيات المسجلة" : "Total Positives Logged"}</p>
            <p className="text-xl font-black text-emerald-900 mt-0.5">{totalGlobalPositives} {isAr ? "نقطة" : "items"}</p>
          </div>
        ...
        <div className="bg-[#FDF3F2] border border-[#F5C6C3] rounded-2xl p-4 shadow-3xs flex items-center justify-between">
          <div>
            <p className="text-[11px] font-extrabold text-rose-800">{isAr ? "مجموع التحديات والسلبيات" : "Total Challenges & Negatives"}</p>
            <p className="text-xl font-black text-rose-900 mt-0.5">{totalGlobalNegatives} {isAr ? "نقطة" : "items"}</p>
          </div>
        ...
        <div className="bg-white border border-[#E2DCC8] rounded-2xl p-4 shadow-3xs flex items-center justify-between">
          <div>
            <p className="text-[11px] font-extrabold text-gray-500">{isAr ? "نسبة التوازن الإيجابي" : "Positive Balance Ratio"}</p>
      ...
      <div className="bg-white border border-[#E2DCC8] rounded-2xl p-4 shadow-3xs flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full md:w-80">
          <input
            ...
            placeholder={isAr ? "ابحث برمز اليوم، التاريخ، أو الكلمات المفتاحية..." : "Search by day key, date, or keywords..."}
        ...
        <div className="flex items-center space-x-2 space-x-reverse w-full md:w-auto justify-end">
          ...
          >
            {isAr ? "الأغلبية إيجابية 🟢" : "Mostly Positive 🟢"}
          </button>
          ...
          >
            {isAr ? "تحديات أعلى 🔴" : "Higher Challenges 🔴"}
          </button>
        </div>
      ...
      {filteredRecords.length === 0 ? (
        <div className="bg-white border-2 border-dashed border-[#E2DCC8] rounded-3xl p-10 text-center space-y-3">
          ...
          <h4 className="text-base font-black text-[#2B3E50]">{isAr ? "لا توجد سجلات مطابقة حالياً" : "No matching records found"}</h4>
          <p className="text-xs text-gray-500 max-w-md mx-auto">
            {isAr ? "انتقل إلى صفحة اليوميات وانقر على أيقونة ⚖️ الإيجابيات والسلبيات بجانب تاريخ أي يوم لحفظ تقرير الإيجابيات والسلبيات الخاص به." : "Go to the Journal tab and click the ⚖️ Pros & Cons icon next to any date to generate its report."}
          </p>
        </div>
      ) : (
                  ...
                  <div className="flex items-center space-x-3 space-x-reverse">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 font-bold">
                        {isAr ? "تاريخ التوثيق:" : "Logged Date:"} {entry.dayKey}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#F2F7F2] border border-[#C2DCBE] rounded-2xl p-4 space-y-2">
                    <div className="flex items-center justify-between border-b border-[#C2DCBE] pb-1.5">
                      <span className="text-xs font-black text-[#2D5A27] flex items-center gap-1.5">
                        ...
                        <span>{isAr ? "الإيجابيات" : "Positives"} ({isAr ? "مجموع " : "total "}{allPositives.length}):</span>
                      </span>
                    </div>
                    ...
                    <ul className="space-y-1.5">
                      ...
                      {allPositives.length === 0 && (
                        <li className="text-xs text-gray-400 italic">{isAr ? "لا توجد إيجابيات مدونة لليوم." : "No positives logged for this day."}</li>
                      )}
                    </ul>
                  </div>
                  ...
                  <div className="bg-[#FDF3F2] border border-[#F5C6C3] rounded-2xl p-4 space-y-2">
                    <div className="flex items-center justify-between border-b border-[#F5C6C3] pb-1.5">
                      <span className="text-xs font-black text-[#902923] flex items-center gap-1.5">
                        ...
                        <span>{isAr ? "التحديات والسلبيات" : "Challenges & Negatives"} ({isAr ? "مجموع " : "total "}{allNegatives.length}):</span>
                      </span>
                    </div>
                    ...
                    <ul className="space-y-1.5">
                      ...
                      {allNegatives.length === 0 && (
                        <li className="text-xs text-gray-400 italic">{isAr ? "لا توجد سلبيات مدونة لليوم." : "No challenges logged for this day."}</li>
                      )}
                    </ul>
                  </div>
